"""
Multi-provider LLM client for Skill Service.
...
class LLMClient:
    ...
    def __init__(self):
        ...
        print(f"[LLMClient/skill] Provider chain: {' → '.join(active)}")
        ...
        for provider, model in self._chain:
            try:
                if provider == "groq":
                    ...
                    return '["ai", "automation", "enterprise"]'  # safe mock
            except _RateLimitError as e:
                print(f"[LLMClient/skill] {provider} rate limited — next. {e}")
                continue
            except Exception as e:
                print(f"[LLMClient/skill] {provider} error: {e} — next.")
                continue

        return '["ai", "automation", "enterprise"]'
