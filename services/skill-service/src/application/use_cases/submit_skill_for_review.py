"""Use Case: Submit a Skill for Review (DRAFT → UNDER_REVIEW)."""

from uuid import UUID

from src.application.dto.skill_dto import SkillDTO
from src.domain.events.skill_events import SkillSubmittedForReview
...
class SubmitSkillForReviewUseCase:
    def __init__(
        self,
    ...
    async def execute(self, skill_id: UUID) -> SkillDTO:
        ...
        event = SkillSubmittedForReview(
            aggregate_id=saved.id,
            skill_id=saved.id,
            organization_id=saved.organization_id,
        )
        await self._event_publisher.publish("skill.submitted_for_review", event)

        # Invalidate cached entries for this skill and org list
