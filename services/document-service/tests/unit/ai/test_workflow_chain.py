...
_GOOD_WORKFLOW = json.dumps([
    {"step_number": 1, "action": "Review contract terms with legal counsel",
     "priority": "High", "description": None, "owner": None, "deadline": None},
    {"step_number": 2, "action": "Sign and return agreement before deadline",
     "priority": "High", "description": None, "owner": None, "deadline": None},
    {"step_number": 3, "action": "Notify legal team of signed agreement",
     "priority": "Medium", "description": None, "owner": None, "deadline": None},
])
...
class TestWorkflowGenerationChain:
    ...
    def mock_llm(self):
        mock = AsyncMock()
        mock.ainvoke.return_value = LLMResponse(content=_GOOD_WORKFLOW)
        return mock
    ...
    @pytest.mark.asyncio
    async def test_ai_wf_004_one_llm_call_made(self, mock_llm):
        """Chain makes exactly one LLM call per run."""
        chain = WorkflowGenerationChain(llm=mock_llm)
        await chain.run(text=_SAMPLE_TEXT)
        assert mock_llm.ainvoke.call_count == 1

    @pytest.mark.asyncio
    async def test_ai_wf_005_bad_json_returns_fallback_steps(self):
        """After max_retries of malformed JSON, returns fallback steps (not raises)."""
        mock_llm = AsyncMock()
        mock_llm.ainvoke.return_value = LLMResponse(content="not json at all")
        chain = WorkflowGenerationChain(llm=mock_llm, max_retries=2)
        result = await chain.run(text=_SAMPLE_TEXT)
        assert isinstance(result, list)
        assert len(result) >= 1
    ...
    async def test_ai_wf_007_out_of_order_step_numbers_are_normalised(self):
        ...
        mock_llm = AsyncMock()
        mock_llm.ainvoke.return_value = LLMResponse(content=scrambled)
        chain = WorkflowGenerationChain(llm=mock_llm)
        result = await chain.run(text=_SAMPLE_TEXT)
        nums = [s["step_number"] for s in result]
        assert nums == list(range(1, len(nums) + 1))
    ...
    async def test_ai_wf_008_fixture_response_validates(self, llm_workflow_response):
        ...
        mock_llm = AsyncMock()
        mock_llm.ainvoke.return_value = LLMResponse(content=llm_workflow_response)
        chain = WorkflowGenerationChain(llm=mock_llm)
        result = await chain.run(text=_SAMPLE_TEXT)
        jsonschema.validate(instance=result, schema=WORKFLOW_SCHEMA)
